...
class AnalyticsEngine:
    ...
    def update(self, metrics: dict):
        ...
        self.current_luggage_count = metrics.get('luggage_count', 0)
    ...
    def _log_to_csv(self, current_count: int, alert_active: bool, polygon_pixels: float = 0, occupied_pixels: float = 0, metrics_capacity: int = 0, area_sq_meters: float = 0):
        ...
        try:
            ...
            # Calculate Used Area
            # Base person area with dynamic buffer
            person_area = current_count * (self.base_footprint * dynamic_buffer)
            
            # Luggage Area (Hybrid: Visual or Statistical)
            luggage_area = 0
            if self.phys_params.get('luggage_enabled', False):
                luggage_size = self.phys_params.get('luggage_avg_area_m2', 0.3)
                
                # Check mode
                if self.phys_params.get('luggage_mode') == 'visual':
                    # Use actual detected count found in step 2.5
                    # metrics is not available here directly, we need to store it in update()
                    # But update() called _log_to_csv, so we need to access the stored value
                    
                    # We need to change the signature of _log_to_csv or store it in self
                    estimated_luggage_count = self.current_luggage_count # Assumes self.current_luggage_count is set
                else:
                    # Statistical fallback
                    luggage_rate = self.phys_params.get('luggage_adoption_rate', 0.7)
                    estimated_luggage_count = current_count * luggage_rate
                
                luggage_area = estimated_luggage_count * luggage_size
            
            used_area = person_area + luggage_area
    ...
    def _log_to_json(self, current_count: int, alert_active: bool, metrics_capacity: int = 0, area_sq_meters: float = 0):
        ...
        try:
            ...
            if self.phys_params.get('luggage_enabled', False):
                luggage_size = self.phys_params.get('luggage_avg_area_m2', 0.3)
                if self.phys_params.get('luggage_mode') == 'visual':
                     estimated_luggage_count = self.current_luggage_count
                else:
                    luggage_rate = self.phys_params.get('luggage_adoption_rate', 0.7)
                    estimated_luggage_count = current_count * luggage_rate
